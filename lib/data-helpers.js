  ...
  [ adminLink ])
    .then(result => {
      ...
      for (let obj in query) {
        ...
        // note: percentage was adjusted to from 100 to 90 to allow for margin
        query[obj][4] = Math.floor(query[obj][1] / max * 90);
        // Transform weighted points to a percentage of total points.
        query[obj][1] = Math.round(query[obj][1] / sum * 100);
      }
      return query;
    })
    .catch(e => {
